 ...
 */
public class MyTickSeekBar extends View {
    ...
    private MyOnSeekChangeListener mSeekChangeListener;
    private Rect mRect;
    ...
    private MySeekParams mSeekParams;
    private int mScale = 1;
    private boolean mAdjustAuto;

    /**
     * Instantiates a new My tick seek bar.
     *
     * @param context the context
     */
    public MyTickSeekBar(Context context) {
        this(context, null);
    }
     ...
     */
    public MyTickSeekBar(Context context, AttributeSet attrs) {
     ...
     */
    public MyTickSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     */
    public MyTickSeekBar(MyBuilder builder) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        MyBuilder myBuilder = new MyBuilder(context);
        if (attrs == null) {
            //setDefaultParams
            this.apply(myBuilder);
            return;
        }
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.TickSeekBar);
        //seekBar
        mMax = ta.getFloat(R.styleable.TickSeekBar_tsb_max, myBuilder.max);
        mMin = ta.getFloat(R.styleable.TickSeekBar_tsb_min, myBuilder.min);
        mProgress = ta.getFloat(R.styleable.TickSeekBar_tsb_progress, myBuilder.progress);
        mIsFloatProgress = ta.getBoolean(R.styleable.TickSeekBar_tsb_progress_value_float, myBuilder.progressValueFloat);
        mUserSeekable = ta.getBoolean(R.styleable.TickSeekBar_tsb_user_seekable, myBuilder.userSeekable);
        mClearPadding = ta.getBoolean(R.styleable.TickSeekBar_tsb_clear_default_padding, myBuilder.clearPadding);
        mOnlyThumbDraggable = ta.getBoolean(R.styleable.TickSeekBar_tsb_only_thumb_draggable, myBuilder.onlyThumbDraggable);
        mSeekSmoothly = ta.getBoolean(R.styleable.TickSeekBar_tsb_seek_smoothly, myBuilder.seekSmoothly);
        mR2L = ta.getBoolean(R.styleable.TickSeekBar_tsb_r2l, myBuilder.r2l);
        //track
        mBackgroundTrackSize = ta.getDimensionPixelSize(R.styleable.TickSeekBar_tsb_track_background_size, myBuilder.trackBackgroundSize);
        mProgressTrackSize = ta.getDimensionPixelSize(R.styleable.TickSeekBar_tsb_track_progress_size, myBuilder.trackProgressSize);
        mBackgroundTrackColor = ta.getColor(R.styleable.TickSeekBar_tsb_track_background_color, myBuilder.trackBackgroundColor);
        mProgressTrackColor = ta.getColor(R.styleable.TickSeekBar_tsb_track_progress_color, myBuilder.trackProgressColor);
        mTrackRoundedCorners = ta.getBoolean(R.styleable.TickSeekBar_tsb_track_rounded_corners, myBuilder.trackRoundedCorners);
        //thumb
        mThumbSize = ta.getDimensionPixelSize(R.styleable.TickSeekBar_tsb_thumb_size, myBuilder.thumbSize);
        mThumbDrawable = ta.getDrawable(R.styleable.TickSeekBar_tsb_thumb_drawable);
        initThumbColor(ta.getColorStateList(R.styleable.TickSeekBar_tsb_thumb_color), myBuilder.thumbColor);
        mAdjustAuto = ta.getBoolean(R.styleable.TickSeekBar_tsb_thumb_adjust_auto, myBuilder.thumbAutoAdjust);
        //thumb text
        mThumbTextShowPos = ta.getInt(R.styleable.TickSeekBar_tsb_show_thumb_text, myBuilder.thumbTextShow);
        mThumbTextColor = ta.getColor(R.styleable.TickSeekBar_tsb_thumb_text_color, myBuilder.thumbTextColor);
        //tickMarks
        mTicksCount = ta.getInt(R.styleable.TickSeekBar_tsb_ticks_count, myBuilder.tickCount);
        mShowTickMarksType = ta.getInt(R.styleable.TickSeekBar_tsb_show_tick_marks_type, myBuilder.showTickMarksType);
        mTickMarksSize = ta.getDimensionPixelSize(R.styleable.TickSeekBar_tsb_tick_marks_size, myBuilder.tickMarksSize);
        initTickMarksColor(ta.getColorStateList(R.styleable.TickSeekBar_tsb_tick_marks_color), myBuilder.tickMarksColor);
        mTickMarksDrawable = ta.getDrawable(R.styleable.TickSeekBar_tsb_tick_marks_drawable);
        mTickMarksSweptHide = ta.getBoolean(R.styleable.TickSeekBar_tsb_tick_marks_swept_hide, myBuilder.tickMarksSweptHide);
        mTickMarksEndsHide = ta.getBoolean(R.styleable.TickSeekBar_tsb_tick_marks_ends_hide, myBuilder.tickMarksEndsHide);
        //tickTexts
        mTickTextsPosition = ta.getInt(R.styleable.TickSeekBar_tsb_show_tick_texts, TextPosition.NONE);
        mTickTextsSize = ta.getDimensionPixelSize(R.styleable.TickSeekBar_tsb_tick_texts_size, myBuilder.tickTextsSize);
        initTickTextsColor(ta.getColorStateList(R.styleable.TickSeekBar_tsb_tick_texts_color), myBuilder.tickTextsColor);
        mTickTextsCustomArray = ta.getTextArray(R.styleable.TickSeekBar_tsb_tick_texts_array);
        initTextsTypeface(ta.getInt(R.styleable.TickSeekBar_tsb_tick_texts_typeface, -1), myBuilder.tickTextsTypeFace);
        ta.recycle();
    }
    ...
    private void collectTicksInfo() {
        ...
        System.out.println(Arrays.asList(mProgressArr));
    }
...
//        initTickXAndTextXLocation();
    ...
    private MySeekParams collectParams(boolean formUser) {
        if (mSeekParams == null) {
            mSeekParams = new MySeekParams(this);
        }
    ...
    private String getProgressString(float progress) {
        if (mIsFloatProgress) {
            return MyFormatUtils.fastFormat(progress, mScale);
        }
    ...
    private void apply(MyBuilder myBuilder) {
        //seek bar
        this.mMax = myBuilder.max;
        this.mMin = myBuilder.min;
        this.mProgress = myBuilder.progress;
        this.mIsFloatProgress = myBuilder.progressValueFloat;
        this.mSeekSmoothly = myBuilder.seekSmoothly;
        this.mR2L = myBuilder.r2l;
        this.mUserSeekable = myBuilder.userSeekable;
        this.mClearPadding = myBuilder.clearPadding;
        this.mOnlyThumbDraggable = myBuilder.onlyThumbDraggable;
        //track
        this.mBackgroundTrackSize = myBuilder.trackBackgroundSize;
        this.mBackgroundTrackColor = myBuilder.trackBackgroundColor;
        this.mProgressTrackSize = myBuilder.trackProgressSize;
        this.mProgressTrackColor = myBuilder.trackProgressColor;
        this.mTrackRoundedCorners = myBuilder.trackRoundedCorners;
        //thumb
        this.mThumbSize = myBuilder.thumbSize;
        this.mThumbDrawable = myBuilder.thumbDrawable;
        this.mThumbTextColor = myBuilder.thumbTextColor;
        initThumbColor(myBuilder.thumbColorStateList, myBuilder.thumbColor);
        this.mThumbTextShowPos = myBuilder.thumbTextShow;
        //tickMarks
        this.mTicksCount = myBuilder.tickCount;
        this.mShowTickMarksType = myBuilder.showTickMarksType;
        this.mTickMarksSize = myBuilder.tickMarksSize;
        this.mTickMarksDrawable = myBuilder.tickMarksDrawable;
        this.mTickMarksEndsHide = myBuilder.tickMarksEndsHide;
        this.mTickMarksSweptHide = myBuilder.tickMarksSweptHide;
        initTickMarksColor(myBuilder.tickMarksColorStateList, myBuilder.tickMarksColor);
        //tickTexts
        this.mTickTextsPosition = myBuilder.tickTextsShow;
        this.mTickTextsSize = myBuilder.tickTextsSize;
        this.mTickTextsCustomArray = myBuilder.tickTextsCustomArray;
        this.mTextsTypeface = myBuilder.tickTextsTypeFace;
        initTickTextsColor(myBuilder.tickTextsColorStateList, myBuilder.tickTextsColor);
    }
     ...
     */
    public static MyBuilder with(@NonNull Context context) {
        return new MyBuilder(context);
    }
     ...
     */
    public MyOnSeekChangeListener getOnSeekChangeListener() {
     ...
     *
     * @param drawable the drawable for thumb,selector drawable is ok.                 selector format:
     */
//<?xml version="1.0" encoding="utf-8"?>
    //<selector xmlns:android="http://schemas.android.com/apk/res/android">
    //<item android:drawable="@drawable/ic_launcher" android:state_pressed="true" />  <!--this drawable is for thumb when pressing-->
     ...
     *
     * @param thumbColorStateList color selector                            selector format like:
     */
//<?xml version="1.0" encoding="utf-8"?>
    //<selector xmlns:android="http://schemas.android.com/apk/res/android">
     ...
     *
     * @param drawable the drawable for marks,selector drawable is ok.                 selector format:
     */
//< ?xml version="1.0" encoding="utf-8"?>
    //<selector xmlns:android="http://schemas.android.com/apk/res/android">
    //<item android:drawable="@drawable/ic_launcher" android:state_selected="true" />  < !--this drawable is for thickMarks which thumb swept-->
     ...
     * set the seek bar's tick's color.
     *
     * @param tickMarksColorStateList colorInt                                selector format like:
     */
//<?xml version="1.0" encoding="utf-8"?>
    //<selector xmlns:android="http://schemas.android.com/apk/res/android">
     ...
     *
     * @param tickTextsColorStateList ColorInt                                selector format like:
     */
//<?xml version="1.0" encoding="utf-8"?>
    //<selector xmlns:android="http://schemas.android.com/apk/res/android">
     ...
     */
    public void setOnSeekChangeListener(@NonNull MyOnSeekChangeListener listener) {
